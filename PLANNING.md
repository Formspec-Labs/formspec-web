...
### FW-0044 — *(closed as live (slice 1); see [## Closed](#closed); production IndexedDB queue + at-rest encryption blocked on FW-0082; service-worker installation + lifecycle UX is FW-0081; Background Sync is FW-0083 (depends on 81 + 82); multi-tab coordination is FW-0084; cross-device queue migration is FW-0085 (depends on FW-0078 + XS-2); definition-drift detection on replay is FW-0086; demo-form offline opt-in is FW-0087 (gated on FW-0082); FW-0080 consolidation trigger fired — FW-0080 now imminent)*
...
- **Phase:** Post-MVP
- **Status:** imminent (trigger fired by FW-0044: sixth `RuntimeFeatureKey` (`offlineSubmit`) landed 2026-05-24)
...
- **Trigger fired (2026-05-24):** FW-0044 landed `offlineSubmit` as the sixth `RuntimeFeatureKey`. Per the row's "pull forward when a sixth `RuntimeFeatureKey` lands or when a fourth `consumes*` flag is about to be added" condition, the trigger fires. FW-0044 explicitly did NOT add a fourth `consumes*` flag (every narrowed route is uniformly `unavailable` on `offlineSubmit`); the sixth-key condition fires standalone. Pull this row forward into the next slice.
- **Blocked on:** Nothing upstream. Refactor is pure internal architectural cleanup.

...
- **Release gaps named:** (a) Form-driven OIDC scope narrowing — `OidcAdapter` still requests `'openid profile email'` blanket; per-form scope gating is a slice-2 follow-on under FW-0028. (b) Per-form assurance step-up — re-discover with the form's actual `requiredAssurance` after form load + step-up affordance for under-assurance claims; depends on queue EXT-8 (form-side assurance annotation); ships with FW-0020. (c) Cross-IdP identity continuity — slot the same user across different IdPs across sessions (FW-0020). (d) Federated identity claim handoff — cross-deployment (FW-0030; depends on SC-4 + EXT-8a). (e) Passkey-first adapter — `PasskeyAdapter` (FW-0031; depends on SC-4 + WebAuthn substrate). (f) Coercion-aware filter — filter IdPs by `duressAware: required` form policy (FW-0048 / FW-0059 build); composite gains an optional adapter capability hint. (g) Long-list affordance — picker is a button list today; grouped-by-assurance or search affordance defers until a real adopter trips the threshold. (h) Config-driven multi-IdP — `config.ports.identityProvider: 'composite'` variant + `config.identity.oidcs: OidcClientConfig[]` shape; slice 1 ships code-level composition only, per project no-shims discipline (no parallel single-+-multi config paths). (i) Form-policy authoring lever — no FEL declaration today for "this form REQUIRES the picker" or "this form FORBIDS anonymous fallback even when the deployment allows it"; today the deployment's `identityMode` decides for every form. Authoring lever wants a per-form override; design lands in slice 2 alongside (b) so the assurance step-up + picker-required surfaces share one runtime resolution path (arch review M-4, deferred).
- **Blocked on:** slice 2 needs queue EXT-8 (form-side assurance annotation) for the form-driven assurance step-up flow; remaining release gaps are tracked above with their owning rows.

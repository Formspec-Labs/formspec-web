...
- **Remaining release gaps:** cross-reload / cross-device draft resume is not server-backed until EXT-26; in-place anonymous draft update is not session-bound server-side until EXT-27, so web creates a fresh anonymous draft per save; full OIDC production submit waits for EXT-23 plus the explicit OIDC sign-in/server-validation proof.

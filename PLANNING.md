...
- **Done:** `src/app/main.tsx` parses the route BEFORE constructing the composition. When `parseStatusRoute(window.location.href)` matches, `main.tsx` constructs a status-only composition via a new `statusRouteComposition()` helper that wires ONLY `statusReader` + the runtime profile / policy resolver (plus the bare-minimum `Composition` slots the React shell unconditionally reads — `mode`, `instanceCapabilities`, `orgRuntimePolicy`). No `IdentityProvider`, no `DraftStore`, no `SubmitTransport`, no `DefinitionSource`, no `FormspecProvider` / formspec-engine init constructed at all. `tests/app/status-runtime.test.tsx#composition narrowing` (new) asserts boot through `main.tsx`'s code path constructs no non-status adapters (mock the adapter constructors and assert they're not called when the URL is `/status?case=...`).
- **Why this matters:** FW-0039 slice 1 closed claiming "the `/status` route composes ONLY runtime profile resolution + StatusReader.readStatus + render." That claim was true at the *consumer* level (`StatusRuntime` does not USE non-status ports at runtime) but false at the *composition* level (`main.tsx` unconditionally calls `createDefaultComposition(activeConfig)` BEFORE the route is read, so every adapter — including `formspec-engine` init — boots regardless). Route-aware composition narrowing closes the gap between the claim and the wiring, making the FW-0039 §Accountless-access §"page renders without an `IdentityProvider` session" line honest at every layer.
- **Consumes ports:** none (refactor of `main.tsx` and `src/composition/`).
- **Blocked on:** no upstream block — filed from FW-0039 Slice 1 closeout independent architecture review H-1.
- **Anti-patterns:** (none — composition refactor).

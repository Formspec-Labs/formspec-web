...
- **Done:** The four sibling-factory families (`createDefault/Stub/Demo` × `App | StatusRoute | ObligationsRoute | DocumentsRoute`) collapse into one parameterized helper (`createRouteNarrowedComposition` or similar) per FW-0068 design §"Sibling future consumers" + design §"Sibling-factory parameterization decision" in FW-0056. The parameterization takes a `routeCite` (`'/status' | '/obligations' | '/documents'`), a list of load-bearing port wirings, and a list of MVP ports to noop. The three `chooseComposition` arms collapse to three callers of one helper. No behavior change; pure internal refactor; coherence assertion still funnels through `freezeComposition`; boot-narrowing tests still pass against the new factory.
- **Consumes ports:** none (pure refactor); shape change in `src/composition/{default,stub,demo}.ts` only.
- **Blocked on:** no upstream block. Pure internal refactor; freely schedulable. Filed from FW-0056 closeout — N=4 sibling families landed inline per FW-0056 design §"Sibling-factory parameterization decision" (option a: file the refactor row, ship the fourth sibling, defer parameterization). The refactor benefits from having all four families landed and stable as evidence — the compression is cleanest when the pattern is observable in code.
- **Scheduling note:** pull forward before a fifth narrowed-route consumer lands — at N=5 the parameterization refactor cost grows substantively (the cruft compounds: every new route adds ~50 lines of sibling-factory boilerplate, the coherence-assertion test matrix scales linearly, and the chooseComposition switch grows another arm). Independent FW-0056 arch review (LOW-2) flagged this as "sooner than typical post-MVP work." Treat the next route-narrowing row (FW-0033 / J-040 upload, any new identity-bound dashboard) as the trigger to land FW-0070 first.
- **Anti-patterns:** (none — internal refactor).

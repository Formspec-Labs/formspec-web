...
import {
  ...
  noopSubmitTransport,
} from '../../src/adapters/noop-for-narrowed-route/index.ts';
import type { FormResponse, IntakeHandoff } from '../../src/ports/index.ts';

describe('noop-for-narrowed-route adapters throw with FW-0068 cite on any call', () => {
  ...
  it('routeCite, when passed, surfaces in the diagnostic so the error names the narrowing route', async () => {
    await expect(noopDefinitionSource('/status').getDefinition('https://x')).rejects.toThrow(
      /narrowed by \/status/,
    );
    await expect(
      noopDraftStore('/obligations').load({ formUrl: 'https://x', subjectRef: 's' }),
    ).rejects.toThrow(/narrowed by \/obligations/);
  });

  it('omitting routeCite still produces the FW-0068 cite (backwards-compatible default)', async () => {
    await expect(noopDefinitionSource().getDefinition('https://x')).rejects.toThrow(/FW-0068/);
  });
});

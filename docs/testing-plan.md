...
| M8 deployment closeout | Docker build, runtime config emission, nginx compression/cache headers, compose config, docs for deferred server stack, operations, deployment, multi-deployment, and README final polish. | `npm run build`, `npm run check:compose-config`, `npm run test:compose-quickstart`, `npm run test:deployment`, `npm run test:multi-deployment`, `./docker-compose.yml`, `README.md`, `docs/deployment.md`, `docs/operations.md`, `docs/multi-deployment.md`. |

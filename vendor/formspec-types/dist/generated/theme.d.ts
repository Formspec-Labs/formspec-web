...
export type PresentationBlock = {
    ...
    /**
     * Widget-specific configuration. Properties depend on the widget. Renderers MUST ignore unrecognized keys. Theme fallback resolution does NOT carry widgetConfig forward — each fallback widget uses its own default configuration — except allowAdd/allowRemove on a repeatable group item, which every rendering widget honors. Component fallback carry/drop/translate policy is defined structurally in specs/ui-policy.json. Repeatable group items (any widget): allowAdd and allowRemove (booleans, default true) lock the add/remove affordances when no Component Document binds the group; presentation-only, they do not change minRepeat/maxRepeat cardinality and every instance supplied by data still renders. TextInput maxLength is a display-only character-count hint that never blocks submission; enforce limits with a Definition Bind constraint such as 'length($) <= 200'. width (TextInput, NumberInput, MoneyInput, DatePicker, Select): one of '2xs'|'xs'|'sm'|'md'|'lg'|'xl'|'2xl', sizing the control to 5/9/13/20/30/40/50 ex respectively (theme-spec.md §4.2 Width Stops) so it means the same under every adapter; absent fills the form column. Well-known configs by widget: TextInput (maxLength, inputMode, rows, maxRows, autoResize, width, placeholder), Text (rows, maxRows, autoResize, placeholder), NumberInput (showStepper, locale, width, placeholder), DatePicker (format, minDate, maxDate, width, placeholder), Select (searchable, width, placeholder), CheckboxGroup (columns, maxVisible), FileUpload (accept, maxSizeMb, preview), MoneyInput (showCurrencySymbol, locale, width, placeholder), Slider (min, max, step, showTicks, showValue), Toggle (onLabel, offLabel), RadioGroup (direction, columns), Signature (strokeColor, height).
     */
    widgetConfig?: {
        [k: string]: unknown;
    };

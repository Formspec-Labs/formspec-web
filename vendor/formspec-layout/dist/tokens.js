 ...
 */
export function resolveToken(val, componentTokens, themeTokens) {
 ...
 */
export function emitMergedThemeCssVars(target, options) {
    const merged = {
        ...(options.themeTokens || {}),
        ...(options.componentTokens || {}),
    };
    const nextKeys = new Set(Object.keys(merged).map((k) => `--formspec-${k.replace(/\./g, '-')}`));

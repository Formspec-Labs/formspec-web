...
export class WebCryptoVerifier {
    constructor(options = {}) {
        this.adapterInfo = {
    ...
    async verify(request, registry) {
        const entry = resolveRegistryEntry(registry, request.methodUri);
        if (!entry) {
            return this.unsupportedReceipt(registry, request, `method not in registry: ${request.methodUri}`);
        }
        if (entry.status === 'deprecated') {
            return this.unsupportedReceipt(registry, request, `method deprecated: ${request.methodUri}`);
        }
        ...
        try {
            ...
            if (e instanceof KeyResolverError) {
                throw new VerifierError(`key resolver: ${sanitizeReason(e.message)}`, 'internal');
            }
            throw e;
        }
    ...
    async dispatchAlg(request, alg, keyBytes) {
    ...
    async verifyEd25519(request, keyBytes) {
        ...
        const key = await importPublicKey(keyBytes, { name: 'Ed25519' }, 'Ed25519');
        ...
        return verdictFromSubtle(async () => {
            ...
            return crypto.subtle.verify({ name: 'Ed25519' }, key, cose.value.signature, sigStructure);
        }, 'Ed25519');
    }
    async verifyEcdsaP256(request, keyBytes) {
        ...
        const key = await importPublicKey(keyBytes, { name: 'ECDSA', namedCurve: 'P-256' }, 'ECDSA-P256');
        const cose = decodeCoseEnvelope(request.signatureBytes, this.methodUriPrefix);
        if (cose.kind === 'unsupported') {
            return cose;
        }
        ...
        return verdictFromSubtle(async () => {
            const payload = resolvePayload(cose.value, request.signedBytes);
            const sigStructure = sigStructureBytes(cose.value.protectedHeaderBytes, payload);
            return crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, key, cose.value.signature, sigStructure);
        }, 'ECDSA-P256');
    }
    async verifyRsaPssSha256(request, keyBytes) {
        ...
        let key;
        try {
            key = await crypto.subtle.importKey('spki', spki, { name: 'RSA-PSS', hash: 'SHA-256' }, false, ['verify']);
        }
        catch (e) {
            throw new VerifierError(`RSA-PSS-SHA256 key import failed: ${sanitizeReason(String(e))}`, 'internal');
        }
        const cose = decodeCoseEnvelope(request.signatureBytes, this.methodUriPrefix);
        if (cose.kind === 'unsupported') {
            return cose;
        ...
        return verdictFromSubtle(async () => {
            const payload = resolvePayload(cose.value, request.signedBytes);
            const sigStructure = sigStructureBytes(cose.value.protectedHeaderBytes, payload);
            return crypto.subtle.verify({ name: 'RSA-PSS', saltLength: 32 }, key, cose.value.signature, sigStructure);
        }, 'RSA-PSS-SHA256');
    }
    unsupportedReceipt(registry, request, reason) {
        return {
            result: 'unsupported',
            method: request.methodUri,
            methodRegistryVersion: registry.version,
 ...
 */
async function resolveKeyBytes(keyRef, resolver) {
 ...
 */
function assertKidBinding(keyRef, coseKid) {
    if (keyRef.kind !== 'kid') {
        return null;
    }
    if (coseKid === null) {
 ...
 */
function assertMethodUriBinding(requestMethod, coseMethodUri) {
 ...
 */
function validateKeyLengthForAlg(alg, keyBytes) {
    switch (alg) {
        case -8:
 ...
 */
async function importPublicKey(keyBytes, algorithm, label) {
    try {
        return await crypto.subtle.importKey('raw', keyBytes, algorithm, false, ['verify']);
    }
    catch (e) {
        throw new VerifierError(`${label} key import failed: ${sanitizeReason(String(e))}`, 'internal');
    }
}
/**
 ...
 */
function decodeCoseEnvelope(signatureBytes, methodUriPrefix) {
    try {
 ...
 */
async function verdictFromSubtle(run, label) {
    let isValid;
    try {
        isValid = await run();
    }
        ...
        throw new VerifierError(`${label} verify crashed: ${sanitizeReason(String(e))}`, 'internal');
    }
 ...
 */
function wrapPkcs1RsaPublicKeyInSpki(pkcs1) {
    ...
    const out = new Uint8Array(outerHeader.length + algorithmIdentifier.length + bitStringHeader.length + pkcs1.length);
    let offset = 0;

...
export const COSE_LABEL_SUITE_ID = -65537;
export const COSE_LABEL_ARTIFACT_TYPE = -65538;
const COSE_LABEL_RETIRED_DISPATCH_ID = -65539;
/**
 ...
 */
export const COSE_LABEL_METHOD_URI = -65540;
...
export function decodeCoseSign1(bytes) {
    ...
    return {
        ...
        unprotectedHeader: unprotectedValue,
        payload,
        signature,
        ...
        methodUri: optionalTextLabel(protectedHeaderValue, COSE_LABEL_METHOD_URI, MAX_METHOD_URI_LEN),
    };
}
/**
 ...
 */
export function decodeCoseSign1WithMethodUri(bytes, expectedPrefix) {
    const cose = decodeCoseSign1(bytes);
    if (cose.methodUri === null) {
        throw new CoseError(`missing method_uri protected header (label ${COSE_LABEL_METHOD_URI})`);
    }
    if (!cose.methodUri.startsWith(expectedPrefix)) {
        throw new CoseError(`method_uri ${JSON.stringify(cose.methodUri)} does not match expected prefix ${JSON.stringify(expectedPrefix)}`);
    }
 ...
 */
export function extractMethodUri(bytes, expectedPrefix) {
    return decodeCoseSign1WithMethodUri(bytes, expectedPrefix).methodUri;
}
export function resolvePayload(cose, detachedPayload) {
...
export function sigStructureBytes(protectedHeader, payload) {
    return concatBytes(new Uint8Array([0x84]), encodeText('Signature1'), encodeBytes(protectedHeader), new Uint8Array([0x40]), encodeBytes(payload));
}
export function protectedHeaderBytesForAlg(alg, kid) {
 ...
 */
export function detachedSignatureProtectedHeader(alg, kid, methodUri) {
    return concatBytes(encodeMajorLen(5, 3), encodeInt(COSE_LABEL_ALG), encodeInt(alg), encodeInt(COSE_LABEL_KID), encodeBytes(kid), encodeInt(COSE_LABEL_METHOD_URI), encodeText(methodUri));
}
export function protectedHeaderBytesWithSuiteId(kid, suiteId = SUITE_ID_PHASE_1) {
    if (kid.byteLength !== 16) {
        throw new CoseError('kid must be 16 bytes');
    }
    return substrateProtectedHeader(-8, kid, suiteId, 'event');
}
export function substrateProtectedHeader(alg, kid, suiteId, artifactType) {
    if (kid.byteLength !== 16) {
        throw new CoseError('kid must be 16 bytes');
    }
    return concatBytes(new Uint8Array([0xa4]), encodeInt(COSE_LABEL_ALG), encodeInt(alg), encodeInt(COSE_LABEL_KID), encodeBytes(kid), encodeInt(COSE_LABEL_SUITE_ID), encodeInt(suiteId), encodeInt(COSE_LABEL_ARTIFACT_TYPE), encodeText(artifactType));
}
export function encodeCoseSign1(protectedHeader, payload, signature) {
    return concatBytes(new Uint8Array([0xd2, 0x84]), encodeBytes(protectedHeader), new Uint8Array([0xa0]), payload === null ? new Uint8Array([0xf6]) : encodeBytes(payload), encodeBytes(signature));
}
export async function deriveKid(suiteId, publicKey) {
    ...
    const digest = new Uint8Array(await subtle.digest('SHA-256', concatBytes(encodeInt(suiteId), publicKey)));
    return digest.slice(0, 16);
}
function optionalIntegerLabel(map, label) {
...
function optionalUnsignedIntegerLabel(map, label) {
...
function optionalBytesLabel(map, label) {
    const value = map.get(label);
    if (value === undefined) {
        return null;
    }
...
function optionalTextLabel(map, label, maxUtf8Bytes) {
    const value = map.get(label);
    if (value === undefined) {
        return null;
    }
...
function rejectRetiredDispatchLabel(map) {
    if (map.has(COSE_LABEL_RETIRED_DISPATCH_ID)) {
        throw new CoseError('RetiredDispatchLabelPresent: retired protected-header label -65539 is present');
    }
}
...
function encodeMajorLen(major, value) {
...
class CborDecoder {
    ...
    read() {
        ...
        switch (major) {
            ...
            case 3:
                return new TextDecoder('utf-8', { fatal: true }).decode(this.readBytes(this.readLen(additional)));
            case 4:

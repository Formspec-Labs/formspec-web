...
The verifier in formspec-web is **one of four distribution modes** per stack-root [ADR-0131 (verifier distribution)](../thoughts/adr/0131-verifier-distribution.md): browser (this repo, post-MVP per web ADR-0005), CLI (`integrity-verify-cli`), embedded-library (`@formspec-org/integrity-signature-*`), reproducible-bundle (deferred). All four are open forks until a trigger fires; formspec-web does not own the verifier surface exclusively.

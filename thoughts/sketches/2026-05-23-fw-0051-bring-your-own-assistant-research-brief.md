...
| Transport: WebMCP binding | §7.2 | `document.modelContext.registerTool()` + `consequentialHint` on write tools, so the browser/agent confirms before running them; the form's confirm handler is the provider-side layer. **Browser-mediated confirmation.** The browser is the trust anchor. |
...
- **What the respondent needs.** The form must (a) declare itself an Assist Provider via WebMCP (`document.modelContext`); (b) expose the full introspection tool catalog; (c) let the assistant invoke profile-match (`formspec.profile.match`) + profile-apply (`formspec.profile.apply` with `confirm: true`); (d) preserve accessibility semantics in parallel (the assistant is an addition, not a replacement, for the screen reader — Assist §8 Passive annotations and standard accessibility metadata MUST coexist).
- **Design posture.** WebMCP is the canonical transport (Assist §7.2). The form's runtime registers tools via `registerTool()`; the extension discovers via `getTools()` + invokes via `executeTool()`; `consequentialHint` plus the form's confirm affordance mediate confirms. **The browser is the trust anchor.** The extension is privileged-browser-context per Assist §11.7; isolated from page-context code. The form trusts the browser's mediation; the form does not trust the extension's identity beyond what WebMCP exposes. **Canonical scenario for the WebMCP path; design supports this directly via Assist §7.2.**
...
1. **Out-of-scope for FW-0051.** The browser's WebExtension permission model + the WebMCP `document.modelContext` API already mediate per-extension consent; FW-0051 trusts the browser's per-extension boundary. Per-assistant revocation = uninstall the extension.
...
### 6.1 WebMCP — `document.modelContext`

- **WebMCP draft / WICG-incubated proposal.** [`https://github.com/webmachinelearning/webmcp`](https://github.com/webmachinelearning/webmcp) — W3C Web Machine Learning CG incubation of `document.modelContext` as a browser-native MCP-style transport. Chrome 149 / Edge 150 origin trials. **Direct binding** for Assist §7.2.

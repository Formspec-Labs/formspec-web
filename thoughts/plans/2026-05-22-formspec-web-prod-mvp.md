...
| B | **Authentication: direct token trust.** formspec-web obtains OIDC tokens via its `IdentityProvider` port; formspec-server validates the token directly against the issuer's JWKS. | No federation, no token exchange. **EXT-23** (server-side per-tenant trusted-issuer config + JWKS client + RS256 verifier) is **verified fully absent in `formspec-server`** as of 2026-05-23 (`formspec-server-auth-jwt` remains HS256-only; `jwks_url` is config-shaped but not consumed into a verifier path). EXT-23 is filed as a peer milestone in `formspec-server`; **M7 acceptance gates on its delivery**. |
...
- [`../specs/2026-05-22-upstream-extension-queue.md`](../specs/2026-05-22-upstream-extension-queue.md) — governs upstream dependencies; EXT-IDs cited here resolve there. New EXT-19..27 reference-deployment gaps (this plan) migrated into the queue at M8.
...
- EXT-19, EXT-20, EXT-21, EXT-22, EXT-23, EXT-24, EXT-25, EXT-26, and EXT-27 migrated into [`2026-05-22-upstream-extension-queue.md`](../specs/2026-05-22-upstream-extension-queue.md) with owning repo, FW rows blocked, fixture status, status (`not yet filed` / `filed` / `landed` / `gates M7` for EXT-23). Owners named where known.
...
**Upstream dependencies:** none new. Surfaces and schedules EXT-19..27; EXT-23 is the hard M7 gate, while EXT-26 and EXT-27 remain release-signoff blockers for draft resume/update semantics.
...
- **EXT-23 (server-side per-tenant trusted-issuer config) is filed as a peer milestone.** Verified 2026-05-23: fully absent in `formspec-server` (no JWKS client, no RS256 verifier, no multi-issuer registry; `JwtAuthConfig.jwks_url` exists but `composition.rs` still sets it to `None`, and `formspec-server-auth-jwt` still constructs HS256 issuers/verifiers). Decision B locked; EXT-23 is substantive new server-side work (issuer registry per-tenant + JWKS client + RS256 verifier path in `stack-common-auth` or `formspec-server-auth-jwt`). **Gates M7 acceptance.** Owner: formspec-server. Schedule: TBD (see §12).
...
- **EXT-19..22 and EXT-26..27 scheduling.** Surfaced as queue items, not gated for local web MVP closeout. But they are *visible* defects in real use:
  ...
  - EXT-26 (`DraftView` has no `draft_state` and no scoped draft lookup) — cross-reload / cross-device draft resume remains same-session only.
  - EXT-27 (`PATCH /drafts/{draft_id}` has no anonymous session token) — anonymous autosave creates a fresh server draft per save instead of in-place updates.
...
| **EXT-23** | Server config + auth | `formspec-server` | **Per-tenant trusted-issuer config + JWKS client + RS256 verifier.** Verified 2026-05-23: fully absent (`formspec-server-auth-jwt` is HS256-only; `jwks_url` remains unconsumed; no JwksClient, no multi-issuer registry, no per-tenant config). Filed as peer milestone in formspec-server. | **Hard: gates M7.** |
...
| **EXT-26** | Server route | `formspec-server` | Server-backed draft state read and resume. Verified 2026-05-23: `DraftView` carries only metadata and not `draft_state`, so web cannot recover draft state without same-session local cache/binding. | Release-signoff blocker for cross-reload / cross-device draft resume. |
| **EXT-27** | Server route | `formspec-server` | Session-bound anonymous draft update. Verified 2026-05-23: `UpdateDraftCommand` carries `draft_state` and optional version only, not `anonymous_session_token`; web avoids the unbound PATCH route for anonymous drafts. | Release-signoff blocker for in-place anonymous draft update semantics. |

...
### 7.2.7 FW-0042 composition (trusted-reviewer — canonical AP-014 vector via reviewer-as-coercer)
...
**Substrate effect:** none on FW-0048's substrate paths — this section is informational, naming a policy-composition recommendation; the duress channel + per-party HPKE wrap + safety-routing adapter are unchanged. The recommended `trustedReviewer: forbidden` posture on this design's template set is a per-form policy declaration enforced at form-load per [web ADR-0011](../adr/0011-runtime-feature-resolution-and-policy-gates.md). Substrate-class promotion of FW-0042's renderer-class panic-revoke into the substrate (auto-revoke timer, safe-template-detection auto-revoke) is filed as FW-0110 per FW-0042 §9.1, blocked on real coercion incident OR adopter request.

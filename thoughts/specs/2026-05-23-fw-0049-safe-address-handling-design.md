...
- **Phase 1 fallback (honest insufficient-coverage):** the receipt omits the safe-* fields entirely (no commitment slot, no manifest entry). **This is a structural tell** — a receipt without an address field reveals that the respondent invoked safe-address protection. Useable for low-risk forms where the tell is acceptable; **NOT acceptable** for J-037's canonical scenarios.
...
6. **PKAF/Rulespec downstream:** when a safe-*-protected value is referenced by a downstream PKAF assertion (e.g., a determination citing residence-eligibility), the assertion's `rkaf:AccessScope` MUST inherit a regulatory-restricted scope reflecting the upstream `accessControl.class`. Specific DPV composition (e.g., `dpv:Location`) is per-deployment; the XS-4 ratification establishes the binding contract.
...
3. **Cascade rule for derived fields (research brief Q6).** A FEL-derived value reading a `safe-address` input — does the derivation automatically inherit the safe-* class? §3.2 proposes `cascade: automatic` for `safe-address` and `safe-contact`, `cascade: manual` for `safe-employer`. **Operational question for the lint regime + Privacy Profile sidecar; FW-0060 build verifies the default behavior matches the registry declaration.**

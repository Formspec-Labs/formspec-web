...
- **Operating against non-WOS-shaped governance.** Per §3 Q3, the form-side runtime trusts the WOS workflow's actor declaration as the authority for "this submission was filed by an agent." Deployments without a WOS-or-equivalent governance layer (where some external substrate emits the actor declaration the form consumes) cannot enforce `aiAgentFiler: allowed | required` honestly — only `aiAgentFiler: forbidden` is meaningfully enforceable in that posture. Non-WOS adopters wire a different runtime feature gate or fork the design.
...
- **Design posture:** **layered defense.** (a) **Bot-protection per AP-019** — privacy-preserving attestation (WebAuthn user-present, Apple Private Access Tokens, Turnstile non-interactive) covers the bulk-submission baseline; an unregistered agent fails. (b) **Identity binding** — an agent attempting to file MUST present a registered agent-identity credential per SC-4 + EXT-8a; unregistered fails. (c) **Form-policy enforcement** — `aiAgentFiler: forbidden` refuses the agent with typed `FeaturePolicyConflictError`. (d) **WOS volume constraints** — registered + authorized agents are bounded by `maxAutonomousPerHour` / `maxAutonomousPerDay` (WOS §11.1); excess escalates to human review. (e) **Honest disclosure** — when the form policy is `forbidden` and the agent attempts to silently submit with forged `capacity: "self"`, the identity-binding chain MUST resolve to a human-class credential; an agent's credential cannot satisfy that. **Composition rule (explicit, not merely enumerative): an unregistered agent MUST fail at least one of layers (a)–(e); a registered+authorized agent succeeds at all five.** The five layers are AND-composed at the submission boundary — bypassing any single layer (e.g., distinguishable IP volume, novel evasion of bot-protection) still leaves the remaining four to fire. Defenses hold at the structural level; the adversarial scraper can ONLY do what (i) bot-protection lets through, AND (ii) identity binding admits, AND (iii) form-policy allows, AND (iv) WOS volume constraints permit, AND (v) honest-credential resolution doesn't catch. **§8 documents the residual risk (forged credentials are an IdP integrity issue, NOT a FW-0058 gap).**
...
| Tier | Semantics |
|---|---|
| `forbidden` | Form REJECTS agent capacity. Submissions claiming `capacity: "ai-agent"` are rejected at TWO points (defense in depth, not alternation): form-load fires `FeaturePolicyConflictError` (form-side, before submit) when the loaded WOS workflow declares an agent actor; submit fires `agentSubmitterUnauthorized` (WOS-side, at submit time, per Kernel §10.5) as the substrate-level enforcement. Both fire when applicable; the form-load check exists to fail fast before the respondent wastes time filling. Default for high-coercion / rights-impacting templates (healthcare consent, advance directive, marriage/divorce, custody, financial POA — the FW-0048 template set). |
...
| `required` | Form REQUIRES agent capacity. Human submissions are rejected. Use case: procurement-automation forms designed exclusively for agent-only filing; ML-system-monitoring forms; agent-to-agent workflow handoff forms. Rare; explicit. **Important: `aiAgentFiler: required` requires the AI agent to BE the signer (capacity `"ai-agent"` per §3.2); it does NOT mean "AI pre-fills the form and a human reviews/signs" — that's FW-0051 (BYO-assistant) + EXT-2 per-field provenance, and the capacity stays `"self"`. The vocabulary distinction is load-bearing; see §7.7.** |
...
**`signerName` and `signerId` convention for agent capacity.** The existing `AuthoredSignature.signerName` is REQUIRED per [`formspec/schemas/response.schema.json:127`](../../../formspec/schemas/response.schema.json). For `capacity: "ai-agent"`, `signerName` SHOULD carry the agent's human-readable display name (e.g., `"Procurement Bot v3"`, `"Acme Tax Filing Agent"`); `signerId` carries the agent's URN (the same URN as `agentChain[0].agentId`). The verifier rendering uses `signerName` as the headline ("filed by Procurement Bot v3") with the chain expansion underneath; this mirrors the human-capacity convention where `signerName` is the human's name and the verifier headlines with it.

**`delegatedBy` URN format.** `delegatedBy` is `string` with `format: "uri"`. Adopters SHOULD use [`WosResourceUrn`-shape URNs](../../../work-spec/schemas/api/_common.schema.json) (e.g., `urn:wos:<tenant>_<entity-type>_<typeid>`) when the deployment is WOS-bound for cross-layer resolver compatibility with WOS Kernel §10.5's `principalActorId` / `delegationRef` substrate ([`work-spec/schemas/api/provenance.schema.json:1796`](../../../work-spec/schemas/api/provenance.schema.json)). Non-WOS deployments MAY use other URN registries (DID-key, custom). The field is intentionally unconstrained-string at the schema level for cross-deployment portability — Formspec does NOT bind WOS-specific URN format on the `AuthoredSignature` envelope per [web ADR-0004 consume-not-invent](../adr/0004-cross-repo-placement-consume-not-invent.md).
...
**In-the-loop disclosure (Art. 22 ¶3 right-to-information at decision time).** Article 22 also confers a right to be informed in advance of solely-automated decisions affecting the data subject. The receipt-side surfaceability above satisfies the post-hoc audit-trail half; the in-the-loop disclosure-to-affected-respondents half (when a human respondent is present in the flow but the agent is the signer — e.g., the healthcare/POA case in §2.3.1, where the patient is affected by the agent's filing on their behalf) is **the deployment's responsibility**. WOS [§12.2](../../../work-spec/specs/ai/ai-integration.md) `discloseThatAgentAssisted: true` is the substrate that drives the disclosure UI; the form runtime composes it. FW-0058 names the seam; the disclosure UI shape itself is out of scope for slice 1 (post-MVP build row).

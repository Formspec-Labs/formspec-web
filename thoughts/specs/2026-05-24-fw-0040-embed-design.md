...
- **Risk:** the runtime's `verifyEmbedOriginAllowed` helper reads from `composition.embedTransport.hostOrigin()` synchronously, but production adapters may need an async handshake before the origin is known. **Catch:** the `EmbedTransport` port has no `init()` hook and `createDefaultComposition` is synchronous; adapters that need an async postMessage handshake (e.g., wrapping `penpal` or `comlink`) cannot complete the handshake before composition construction. The supported pattern: production adapters return `null` from `hostOrigin()` until the handshake completes; the runtime gate fails-closed on `null` (no origin matches the allow-list); the host page is expected to set CSP `frame-ancestors` as defense-in-depth. Adopters who need pre-resolved origin SHOULD complete the handshake in their adapter factory (async function returning `Promise<EmbedTransport>`) BEFORE passing the result into `createDefaultComposition` — but the composition factory itself stays sync. The optional `EmbedTransport.ready?: () => Promise<void>` hook is filed as FW-0102 follow-on; not needed until a consumer requires it.

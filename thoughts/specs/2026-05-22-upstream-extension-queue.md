...
**Fixture status:** schema landed; fixture presence at `formspec/tests/fixtures/items/{consequences,purpose}/` not yet verified — see fixture-debt triage follow-up.
**Status:** **closed 2026-05-26 — schema + docs already landed.** Verified: `formspec/specs/core/spec.md` §4.2.1 lines 2690–2696 carries the explicit retired-`privacy` paragraph (cites EXT-1 by ID, redirects to `accessControl.class` + Privacy Profile / Access-Class Registry). Schema state at `formspec/schemas/definition.schema.json`: `PurposeMetadata` ($defs line 757), `ConsequencesMetadata` ($defs line 655), both attached to Item (line 854); no `privacy` property anywhere — the never-existed block was never written. Scope reduced 2026-05-23 by FW-0049 design (`privacy` retired; safe-address moves to EXT-31 + EXT-32).
...
**Closes:** J-016 (withdraw / dispute / consent revoke), J-026 (decline as a first-class event), J-030 (deletion receipt), J-033 (bot protection cleared), J-017 (disclosure presented). _J-027 (duress signal) retired here 2026-05-26 with ADR-0156 withdrawal — see EXT-30 / XS-3._
**FW rows blocked:** FW-0007, FW-0026, FW-0036, FW-0038, FW-0043, FW-0049 _(FW-0048 withdrawn 2026-05-26 with stack-root ADR-0156)_
**Events to add:** `response.declined` (with optional `clauseReferences[]`, `reason`), `response.withdrawn`, `response.dispute-attached`, `consent.revoked`, `data.erased`, `disclosure.presented`, `field.flagged-by-respondent`, `bot-protection-cleared`. File as one combined PR. _`submission.duress-signaled` was originally Batch B of this entry but was retired 2026-05-26 when its parent ADR-0156 was withdrawn — see EXT-30 / XS-3._
...
**`bot-protection-cleared` payload shape — closed 2026-05-24 by [FW-0036 design §4.4](2026-05-24-fw-0036-humane-bot-protection-design.md).** Payload `data` block: `{attesterTier: "device-attested" | "private-token" | "invisible-challenge" | "human-confirmation" | "escape-to-support", attesterId: string (opaque URN naming the attester adapter, e.g. "urn:formspec:bot-attester:cf-turnstile@1"), outcome: "human" | "agent-registered" | "uncertain" | "denied", evaluatedAt: string (RFC 3339), evidenceRef?: string (opaque attester-side audit id — MUST NOT carry IP, User-Agent, device fingerprint, geolocation, behavioral biometrics or any re-identifiable signal per FW-0036 §6 disclosure obligation)}`. **Presence-encoded, NOT uniform-shape:** unlike `submission.duress-signaled` (which MUST emit on every high-risk-template submission to avoid event-existence leaking the binary signal), `bot-protection-cleared` is emitted when the gate ran (`form-policy: allowed | required`) and absent when the gate did not run (`form-policy: forbidden`); no information leak via event-presence because the form's policy is public (`definitionUrl` + `definitionVersion`). Fixture coverage MUST include: positive `device-attested` + `human` (Tier 1 WebAuthn user-present); positive `invisible-challenge` + `human` (Tier 3 Turnstile-class); positive `agent-registered` (FW-0058 composition; `aiAgentFiler: allowed | required` short-circuit per FW-0036 §7.2); negative `escape-to-support` + `denied` (user invoked Tier 5; form refused with `BotProtectionDeniedError`).
**`submission.duress-signaled` payload shape — WITHDRAWN 2026-05-26 with ADR-0156.** Originally extended by FW-0048 design §5.1; uniform recipient-handle discipline added by ADR-0156 review remediation. Both ADR-0156 and the dependent shape are withdrawn; if a real adopter ships a high-coercion-risk template and surfaces concrete requirements, a new payload-shape entry can be filed from observed needs. See `feedback_survivor_safety_framing_trap` for the framing discipline confirmed by this withdrawal.
**Fixture status:** Batch A landed 2026-05-26 (commit bd1964ca in formspec; 7 events × ≥1 fixture each in `formspec/tests/conformance/fixtures/respondent-ledger/`). The `submission.duress-signaled` fixture sub-clause is retired with ADR-0156 withdrawal.
**Status:** **Batch A landed 2026-05-26** (7 of the originally-proposed 9 events shipped with schema + spec §8.7–§8.13 + 7 fixtures; `bot-protection-cleared` was already shipped pre-this-batch; `submission.duress-signaled` was retired with ADR-0156). Per-party `partyRef` extension landed for `response.withdrawn` / `response.dispute-attached` per FW-0034 design §6.2. Entry is effectively closed — leaving the row in place until a follow-up audit confirms no further sub-clause work remains.
...
**FW rows blocked:** FW-0056 (respondent-side document library build). _(FW-0059 coercion-aware signing build withdrawn 2026-05-26 with stack-root ADR-0156.)_
...
**Cross-stack:** XS-1 ratified 2026-05-26 as stack-root [ADR-0155 (multi-party intake)](../../../thoughts/adr/0155-multi-party-intake.md) `accepted` — spans formspec + WOS + trellis. Trellis substrate is unchanged (sequential per-party signing over digest-stable Formspec Signed Response Payload already works). XS-1 also carries a paired `response.schema.json` extension for disagreement-as-state (per FW-0050 design §5.4 + §6.2 (5)) so conflicting per-party field values can be carried with party attribution instead of silently merged.
...
**Status:** proposed 2026-05-23 by FW-0050 design; XS-1 gate cleared 2026-05-26 by ADR-0155 ratification (stack commit cbb43c7) — pending formspec-side schema authoring.
...
**Cross-stack:** lands as part of XS-3 ratification (see below). Trellis substrate is unchanged for the base duress pipeline — the documented Phase 1 envelope (Core §6.4 `payload_ref` + §9.4 `key_bag` HPKE Base-mode wrap to per-recipient public keys, suite 1) carries the duress payload directly; no §13 commitment-slot use is required (per Core §13.3 Phase 1 producers MUST emit `commitments` as null/[]). ADR 0156 adds the recipient-handle side-channel guard: cleartext `key_bag` recipient handles and event payload handles use a uniform opaque profile handle whenever role-specific routing would reveal sensitive party information, with role-to-recipient fan-out only after authorized safety-service decrypt.
**Fixture status:** none. Retired with the parent ADR; no fixtures will be authored.
**Status:** **withdrawn 2026-05-26** — parent stack-root [ADR-0156 (coercion-aware-signing-pipeline)](../../../thoughts/adr/0156-coercion-aware-signing-pipeline.md) withdrawn the same day. EXT-30 has no consumer without ADR-0156 / FW-0048 / FW-0059. If a real adopter ships a high-coercion-risk template and surfaces concrete routing requirements, a new sidecar block can be filed from observed needs.
...
**Shape:** per [FW-0048 design §6.5](2026-05-23-fw-0048-coercion-aware-signing-design.md) and stack-root ADR 0156: (1) Trellis Core §6.4 + §9.4 carry the base envelope unchanged (signed event, `payload_ref` ciphertext, `key_bag` HPKE wrap to safety-team recipient) — no new substrate primitive required; (2) event-type registration of `submission.duress-signaled` in the bound registry per Core §6.7 + §14 is the only Trellis-side Phase 1 work; (3) advanced post-hoc selective-disclosure manifests over the duress payload would require Phase 2+ (OC-26 + OC-27 + OC-30 + Phase 2 commitment scheme) — explicitly deferred, base pipeline does not depend on it; (4) WOS `safety-reviewer` actorExtension is OPTIONAL — `issuer-webhook` routing doesn't require WOS at all; (5) receipt discipline — verifier MUST NOT distinguish duress-signaled and non-duress submissions in any user-visible way (byte-identical receipt path per §3.2); (6) per-party scoping per [FW-0050 §7.2](2026-05-23-fw-0050-multi-party-submission-design.md) and [FW-0048 §7](2026-05-23-fw-0048-coercion-aware-signing-design.md) — Party B's duress signal MUST NOT be observable to Party A through any surface (status / receipt / ceremony); (7) cleartext recipient handles MUST NOT reveal party, role, or duress-routing differences, using a uniform opaque profile handle whenever role-specific routing would leak.
**Fixture status:** none. Retired with the parent ADR.
**Status:** **withdrawn 2026-05-26** — stack-root [ADR-0156 (coercion-aware-signing-pipeline)](../../../thoughts/adr/0156-coercion-aware-signing-pipeline.md) was withdrawn (commit bac4b65). Framing inflated priority through "survivor-safety load-bearing" language on a greenfield surface with no consumer; see `feedback_survivor_safety_framing_trap`. If a real adopter ships a high-coercion-risk template and surfaces concrete requirements, a new XS / ADR entry can be filed from observed needs rather than synthesized adjacent threat models.
...
6. __Safe-_ composition._* Per [FW-0049 §7](2026-05-23-fw-0049-safe-address-handling-design.md): correction event payloads inherit field `accessControl.class`; correction `reason` and dispute `statement` text carry their own class declaration; receipt-chain renderer honors mask discipline.
...
**Fixture status:** none. Cross-stack confirmation absorbed into ADR-0066 §D-6 (no separate ADR file). Per FW-0034 §5 the fixture matrix lives in FW-0038 build (three-act scenarios + the multi-party + safe-* composition variants).
**Status:** proposed 2026-05-24 by [FW-0034 design](2026-05-24-fw-0034-honest-correction-path-design.md); XS-5 gate cleared 2026-05-26 — absorbed into stack-root [ADR-0066 §D-6](../../../thoughts/adr/0066-stack-amendment-and-supersession.md) (the prior ADR-0158 lifecycle-decision draft collapsed in); pending EXT-5 + EXT-35 schema authoring.
...
7. __Safe-_ composition._*Per [FW-0049 §3.3](2026-05-23-fw-0049-safe-address-handling-design.md): safe-* class fields render masked to the agent's introspection identically. The agent submits without seeing the plaintext; the receipt carries the safe-* class declaration unchanged.
...
**Cross-stack:** XS-5 absorbed 2026-05-26 into stack-root [ADR-0066 §D-6 (amendment + supersession)](../../../thoughts/adr/0066-stack-amendment-and-supersession.md) — the prior ADR-0158 lifecycle-decision draft collapsed into ADR-0066 §D-6 in the same session; three-act mapping across formspec + WOS + trellis is now confirmed there.
...
**Status:** proposed 2026-05-24 by [FW-0034 design](2026-05-24-fw-0034-honest-correction-path-design.md); XS-5 gate cleared 2026-05-26 by ADR-0066 §D-6 absorption (ADR-0158 collapsed in) — pending WOS-side schema authoring.

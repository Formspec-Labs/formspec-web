...
No new feature key. No expansion of the closed taxonomy. **Crucially**, `StatusRuntime` does NOT synthesize a `form: { status: 'required' }` policy at the route boundary — that would manufacture a typed `UnsupportedRequiredFeatureError` on every unavailable-instance load and abuse the form-load error boundary semantics for a non-form surface (arch-review F-4). The status route is an OPTIONAL surface: when the deployment can serve it, the page renders; when it cannot, the page says so. The resolver runs with `form: { features: {} }` and the natural `disabled, cause = 'optional-no-instance' | 'org-forbidden' | 'form-forbidden'` branches drive the not-shared copy. No `RuntimePolicyErrorPage`; no typed error path; no synthetic required-policy. The honesty seam is the same M-3 plumbing FW-0065 shipped.

...
**RESOLVED 2026-05-25 — original proposal won unconditionally.** The FW-0115 Spike-and-Observe pass per [ADR-0155 §8](../../../thoughts/adr/0155-multi-party-intake.md) measured two new-port candidates (Shape A 2PC-explicit-state; Shape B eventual-consistency) against the §8.4 fixtures; neither displaced this section's "extend three existing ports" approach for the §8.9-scoped intake-tier coEqual flow. FW-0061 shipped on §3.2 at commit `fb142c4`: `DraftStore.partyRef` extension landed (see [`src/ports/draft-store.ts:22-26`](../../src/ports/draft-store.ts)); `IdentityProvider` and `SubmitTransport` extended per this section's substrate-port rationale; the sequential-signer FSM is hand-rolled in the consumer at [`src/app/respondent-flow.ts`](../../src/app/respondent-flow.ts). **No new port minted.** An initial close-out locked Shape A as `src/ports/multi-party-commit.ts` (commit `a2f340f`); subsequent verification found the port had no production consumer and it was deleted per DEVELOPMENT-PHILOSOPHY.md "throw away the loser". See [ADR-0155 §8.10](../../../thoughts/adr/0155-multi-party-intake.md) + observation report §5 for the architectural insight (when a 2PC-shaped coordination port would become load-bearing — threshold signatures, DKG, MLS, MPC).
...
2. **Per-party duress channel** _(FW-0048 / FW-0059 / stack-root ADR-0156 withdrawn 2026-05-26 — historical design below; per-party invisibility invariants remain in ADR-0155 §10 if re-filed)_. Party B's duress signal must not be observable to Party A. The `submission.duress-signaled` event from [EXT-5 in `2026-05-22-upstream-extension-queue.md:73`](2026-05-22-upstream-extension-queue.md) lands in a private-sidecar per `trellis-operational-companion.md` §13 — for multi-party, the per-party-private sidecar discipline must be **per-party**, not per-form. **[FW-0048 design `thoughts/specs/2026-05-23-fw-0048-coercion-aware-signing-design.md`](2026-05-23-fw-0048-coercion-aware-signing-design.md) is the canonical sidecar shape; §7 of that design satisfies this delegation explicitly** — `partyRef?` field on the EXT-5 payload, per-party invisibility guarantees at receipt / status / ceremony surfaces, and FW-0061 build constraints (per-party safety-routing dispatch via the deferred adapter shape per FW-0048 §4.2, per-party HPKE-wrap via Trellis Core §6.4 `payload_ref` + §9.4 `key_bag`, multi-party fixture coverage matrix). FW-0061 reads FW-0048 §7 directly and wires the per-party composition; no shape-invention here.
...
- **FW-0058 (AI-agent filer chain) — RESOLVED 2026-05-24.** [FW-0058 design §7.4](2026-05-24-fw-0058-ai-agent-filer-chain-design.md) explicitly satisfies the multi-party composition: an agent filing as one party in a multi-party flow carries BOTH `capacity: "ai-agent"` + `agentChain` AND `partyRole` on the same `AuthoredSignature`. The existing §7.1 per-party-visibility primitive covers; per-party scoping applies to the agent's WOS-governed introspection identically (Party B's agent doesn't see Party A's fields). FW-0050's existing substrate is sufficient; no §7.x carve-out for FW-0058 needed. **Per-party-action symmetry holds**: per-party signing order, per-party draft state, per-party signatures all apply identically when a party is an agent.
- **FW-0051 (bring-your-own-assistant) — RESOLVED 2026-05-23.** [FW-0051 design §7.1](2026-05-23-fw-0051-bring-your-own-assistant-design.md) explicitly satisfies the multi-party composition: when a form declares both `multiParty` AND `bringYourOwnAssistant`, the Assist Provider's tool catalog MUST be per-party-scoped — introspection tools (`formspec.form.describe` / `field.list` / `field.describe` / `field.help` / `form.progress`) return data scoped to the requesting party's `visibleTo[]` per §7.1 above; mutation tools (`formspec.field.set` / `bulkSet`) reject writes outside `editableBy[]`; fields outside party visibility return `NOT_FOUND` per Assist §4.2 (structurally invisible — treated as nonexistent). **FW-0050's existing §7.1 per-party-visibility primitive covers this without a §7.x carve-out;** FW-0051 design names the binding explicitly. **FW-0061 build consumes the FW-0051 §7.1 shape directly** when both capabilities are enabled — no shape-invention needed.
...
3a. **FW-0048 per-party duress sidecar — RESOLVED 2026-05-23.** FW-0048 design landed at [`thoughts/specs/2026-05-23-fw-0048-coercion-aware-signing-design.md`](2026-05-23-fw-0048-coercion-aware-signing-design.md) with §7 explicitly satisfying this row's §7.2 delegation: `partyRef?` field on the EXT-5 payload, per-party HPKE wrap per Trellis Core §9.4 suite 1, per-party invisibility across receipts / status reads / ceremony state, and the FW-0061 build-constraint matrix (per-party `SafetyRouting.dispatch()` retry semantics, per-party commitment-slot population per OC-26, multi-party fixture coverage). FW-0061 consumes the FW-0048 shape directly; **the design dependency closes here**. Build-time dependency on FW-0048's build row (FW-0059 in `PLANNING.md`) remains: FW-0059's `SafetyRouting` adapter must exist before FW-0061 can ship the per-party duress flow, OR FW-0061 must descope the duress affordance from its initial slice.
4. **Sync co-fill** (Q2 deferral). Separate design row when pulled forward.
5. **AI-agent co-party — RESOLVED 2026-05-24.** [FW-0058 design landed](2026-05-24-fw-0058-ai-agent-filer-chain-design.md) with §7.4 satisfying this row's composition: agent as one party carries BOTH `capacity: "ai-agent"` + `agentChain` AND `partyRole` on the same `AuthoredSignature`; per-party visibility per §7.1 applies identically. FW-0061 build consumes the FW-0058 shape directly when both capabilities are enabled.
...
8. **Reviewer-vs-party single-sidecar question (FW-0042 interaction) — RESOLVED 2026-05-25.** Per [FW-0042 design §6.4](2026-05-25-fw-0042-trusted-reviewer-design.md): **two separate sharing primitives, not one** — SC-6 reviewer-thread and `parties[]` co-party state are architecturally independent. The reviewer is READ + COMMENT (+ SUGGEST when allowed) on the respondent's behalf and never holds party identity; a co-party is a distinct first-class party that reads + edits its own scope + signs its own scope. Different capabilities, different trust boundaries, different carriers — SC-6 sidecar (off the signed Response) for the reviewer; `parties[]` per this design + EXT-28 (on the Definition + per-party signatures) for the co-party. A single sidecar would conflate two trust boundaries; the two compose orthogonally per FW-0042 §6.4 multi-party composition.
...
| No new `MultiPartyOrchestrator` port (§3.2) | RESOLVED 2026-05-25 — original proposal won unconditionally. FW-0115 spike measured Shape A vs Shape B against §8.4 fixtures; neither displaced this section. FW-0061 shipped on §3.2 (commit `fb142c4`) with the sequential-signer FSM hand-rolled in `src/app/respondent-flow.ts`. No new port minted; the orphan locked port from initial close-out (`a2f340f`) was deleted. See ADR-0155 §8.10 + observation report §5 | n/a |
| Extend `DraftStore` + `IdentityProvider` + `SubmitTransport` (§3.2) | RESOLVED 2026-05-25 — `DraftStore.partyRef` extension landed (`src/ports/draft-store.ts:22-26`); `IdentityProvider` and `SubmitTransport` extended per §3.2 substrate-port rationale | n/a |

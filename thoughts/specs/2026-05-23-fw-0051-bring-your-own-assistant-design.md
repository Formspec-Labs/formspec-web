...
- **Required:** the form registers the Assist Provider tools via `document.modelContext.registerTool()` (Assist §7.2); the extension discovers via `getTools()` + invokes via `executeTool()`; the browser/agent's `consequentialHint` gate and the form's own per-act confirm affordance mediate confirms; accessibility semantics coexist (the screen reader continues to read the DOM; the assistant uses the tools — both work).
- **Design posture:** WebMCP is the canonical transport; the form's posture toggle activates the Provider; per-act confirm gates are the form's own affordance (Assist §7.1(4) provider layer), with the browser/agent's `consequentialHint` gate underneath; per-field reveal escalation is a respondent-side UI affordance the extension can trigger via tool invocation (FW-0051 §3.3 + §6.2 proposed upstream extension). **Canonical WebMCP scenario; design supports directly via Assist §7.2.**
...
| WebMCP transport adapter (default) | Registers tools via `document.modelContext.registerTool()` per Assist §7.2 with `consequentialHint` on every write; the form's confirm affordance is the provider-side gate. The provider never installs a polyfill (Assist §7.2); the shell, as host page, MAY load one when native WebMCP is unavailable. |

...
- **The `IssuerProvider` port is mostly already implemented inside the engine.** Cross-stack inventory (2026-05-22) confirmed that `formspec/packages/formspec-engine/src/issuer/IssuerStore.ts` ships the full cascade: host override / Definition declaration / fallback resolution, `parentOrganization` chain walk capped at `MAX_CHAIN_DEPTH=8`, cycle guard, ETag-respecting `FetchIssuerFetcher`. formspec-web does NOT build a new port from scratch — it consumes `IssuerStore` and configures the fetcher (caching, CORS-safety, server-proxy per web ADR-0008 if needed).
- **`@formspec-org/react` ships `<Issuer>` component** at `formspec/packages/formspec-react/src/issuer.tsx` — the cover-page chrome consumer. FW-0004 (first-paint legitimacy) imports `<Issuer>`, styles it for the formspec-web brand, and lets the engine's `IssuerStore` do the resolution. Port-shaped work that the original §Decision implied is moot; the React surface is already built.

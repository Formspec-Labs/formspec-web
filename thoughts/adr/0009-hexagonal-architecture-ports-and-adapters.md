...
**(d) Stack-common wire shapes are upstream primitives.** `StackError` / Problem JSON envelope (`stack-common-error`), TypeID format + URN parsing (`stack-common-typeid`), idempotency-key conventions (`stack-common-idempotency`), `ProofReportVerdict` (`stack-common-proof`), request-id header conventions (`stack-common-http`) are cross-stack wire shapes owned by `stack-common/` (Rust). formspec-web's TS adapters consume them via TS mirrors tracked in the upstream extension queue (`EXT-11` ProofReportVerdict, `EXT-12` Problem JSON, `EXT-13` TypeID, `EXT-14` UUIDv7 idempotency-key conventions). **stack-common is Rust-only today** — no `packages/` directory — though the integrity-stack precedent (which ships both `crates/` and `packages/`) suggests a future cross-language pattern. Whether stack-common adopts that pattern is a stack-level architectural decision; formspec-web's queue flags the dependency without owning the fix. Until then, each TS client (formspec-web, formspec-cloud, formspec-studio) mirrors independently with drift risk; the TS mirrors formspec-web ships should be small and conformance-fixtured so cross-stack consolidation later is mechanical.

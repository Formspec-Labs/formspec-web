...
Some feature keys require more than one port. `trustedReviewer` is the first ratified multi-port binding: it is available only when both `ReviewerSession` and `ReviewThreadStore` are wired. A one-slot shortcut is forbidden because minting/revoking reviewer access without a durable review thread, or storing a thread without scoped reviewer sessions, overclaims the feature and violates the fail-closed rule above. `preparerFiling` is an unavailable-only reserved key until FW-0037 lands its concrete filer-session and signer-handoff ports.
...
- Design proposal: [`thoughts/specs/2026-05-24-fw-0037-filer-not-signer-design.md`](../specs/2026-05-24-fw-0037-filer-not-signer-design.md) — ratifies `preparerFiling` as the FW-0037 sibling key under the split-keys discipline. Production enablement waits for the FW-0037 build to land concrete SignerHandoff / filer-session ports.

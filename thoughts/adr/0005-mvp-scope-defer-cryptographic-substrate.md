...
- **Browser verifier:** the Phase-1 COSE_Sign1 verifier ships pure-TS, no WASM (`@formspec-org/integrity-signature-adapter-webcrypto` + `@formspec-org/integrity-cose` + Formspec wrappers `@formspec/signature-*`). The originally-feared "Trellis verifier WASM build + size budget" concern applies only to BBS+ / ECDSA-SD selective-disclosure cryptography, not to MVP-era verification.
